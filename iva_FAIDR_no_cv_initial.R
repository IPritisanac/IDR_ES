...
target_file<-"HIGH_AUC_PPV_FILTERED_FUNCTIONS/GO0005730_nucleolus.txt"
...
y<-targets[,3]
category_name<-sub("\\.txt$", "", basename(target_file))
...
for (rep in 1:N_reps) {
  ...
  roc_cv<-roc(as.numeric(y[which(!is.na(unbiased_pr))]),as.numeric(unbiased_pr[which(!is.na(unbiased_pr))]))
  plot(roc_cv)
  classi_info[1,5] <- classi_info[1,5] + as.numeric(auc(roc_cv))/N_reps;
