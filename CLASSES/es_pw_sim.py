...
class EVSign():
	...
	def __init__(self,inputf):
		...
		# this should be inherited as well
		# //TO DO// IP: Inherit from DATA and from SEQ FEAT to make this a super class
		#self.get_functions()
	...
	def parse_param_files(self,input_file):
		...
		PP.read_in_files()
		# Parse parameters
		# make Parser objects 'self'
		# //TO DO// IP: inherit Parser instead of doing the below
		...
		#return P.motifs_file, P.repeats_file, P.aa_freq_file, P.aln_dir
	...
	def get_features(self,):
		# //TO DO// IP: Make a super constructor that inherits INIT of seq features
		SF=SequenceFeatures()
	...
	### should be able to take a command line number or string to specify the reference species in the alignment.###

	def sim_multi_prot_given_anc(self,seq,d,freq,n): ##simulate n sequences from seq
		...
		for i in range(len(pos_array)):
			if pos_array[i]>0:
				for j in range(0,pos_array[i]):
					...
					#print(i,":",cnt1,size_array[size_indx])
					if itype=='ins':
	...
	def indel_poisson(self,d,seq):
		...
		return r
		#alternative solution -- but this can never get us more than 1 indel at position
		#p_ind=1-(np.e**-(indel_rate)) # probability of 1 or more indels at position
		#options=[0,1]
		#probs=[1-p_ind,p_ind]
		#print(probs)
		# construct process in a single shot
		#indel_pos=np.random.choice(options,len(seq),p=probs)
		#print(indel_pos)
	## IP 9/2/2022
	...
	def compute_es_dir(self):
		...
		for mfa in os.listdir(self.aln_dir):
			...
			#print("\t".join([str(f)+"_meanZ" for f in featnamesorted]))
			#outseq=open(self.outdir+"_SEQ_INDELS_"+str(self.use_indels)+".out.txt","a")
			outparam=open(self.outdir+"_ZSCORE_LIMITS_TEST_INDELS_"+str(self.use_indels)+".out.txt","a")
			#print(self.outdir+"_ES_INDELS_"+str(self.use_indels)+".out.txt")
			...
			#outseq.write("%s\n"%(str(len(ALN.names))+' sequences read in '+mfa+' reference is '+ALN.names[REF_NUM]+" length="+str(len(refs))))
			##use thes for normalization to help avoid zeros in the variances
			...
			#outseq.write("%s\t%s\n"%(mfa,len(sequences_to_use)))
			#for useq in sequences_to_use:
			#    outseq.write("%s\n"%(useq))
			#outseq.write("\n\n")
			print(str(len(sequences_to_use))+' sequences passed filtering')
			#outseq.write("%s\n\n"%(str(len(sequences_to_use))+' sequences passed filtering'))
			for s in sequences_to_use:
			#for s in ALN.names:
			#    if (s is ALN.names[REF_NUM]): continue
				#print(">%s"%(s))
				#print(ALN.seq[s])
				ugseqs[s]=ALN.seq[s].replace("-","")
		        ###do some quality control on which sequences we include -- qualitycontrol is now done in seq choosing heuristic
		    #    if ((len(thisseq)<10) or ('X' in thisseq)):
		    #        continue
		    #    ugseqs[s]=thisseq
			...
			for i in range(0,self.n_sim):
				SIM[i] = Alignment()
				#SIM[i].names = ALN.names[1:]
			for s in ALN.names:
				if (s == ALN.names[REF_NUM]):
					continue
				#eprint(str(s) + " from " + mfa)
				#dis =JC69_aa_dis(ALN.seq[ALN.names[0]],ALN.seq[s])
				...
				for rs in self.sim_multi_prot_given_anc(refs,dis,self.aa_freq,self.n_sim):
					...
					i+=1
				#for i in range(0,N):
				#    SIM[i].seq[s]=sim_prot_given_anc(refs,dis,disofreq)

			print(str(self.n_sim)+ " simulations completed for " + mfa)

			# //TO DO// IP - turn these into nd-numpy arrays
			sm ={}
			sv ={}
			obs[mfa] = {}
			obs_b[mfa] = {}
			...
			for s in ALN.names:
				...
				for m in list(self.motifs.keys()):
					...
					if len(pat.findall(ugseqs[s]))>0:
						#print("FILLING UP OBSERVED WITH MOTIF %s"%(m))
			...
			for i in list(SIM.keys()):
				...
				for f in self.aafeats_names + list(self.repeats.keys()) + list(self.motifs.keys())+ self.seqfeats_names:
					sm[f].append(float(0)) #don't keep the values from the simulations
					sv[f].append(float(0))
				for s in SIM[i].names: #OK to have different numbers of sequences in each simulation
					...
					if not self.use_indels:
						...
						# updated aas to take into account lenght difference **if indels not used**
					...
					for r in list(self.repeats.keys()):
						repeatlength=0
						#if (self.repeats[r][0] in SIM[i].seq[s]):
						#pat = re.compile(self.repeats[r][0]+"{2,}")
						repregex=r.split("m=")[-1].replace('"','')
						pat = re.compile(repregex)
					...
					for m in list(self.motifs.keys()):
						...
						#if len(pat.findall(SIM[i].seq[s]))>0:
							#print(m,SIM[i].seq[s],val)
						sm[m][-1]+= val
						sv[m][-1]+= val*val
				for f in self.aafeats_names + list(self.repeats.keys()) + list(self.motifs.keys()) + self.seqfeats_names:
					...
					if (sv[f][-1]>float(0)):
						...
						#if (i<100): eprint("no variance in sim" +str(i) +" for " + f)
						sv[f][-1] = np.nan
			...
			for f in self.aafeats_names + list(self.repeats.keys()) + list(self.motifs.keys()) + self.seqfeats_names:
				...
				if ((simsd=="") or (not simsd > self.min_sd)):
					...
					if (f in self.motifs.keys()) and (mz>100 or mz<-100):
						...
						if limit_test: # giving an arbitrarily large mean and var score
							outparam.write("LIMIT CASE TRUE FOR %s"%(f))
							Z[mfa][f+" meanZ"]=Zscore_mean
							Z[mfa][f+" varZ"]=Zscore_var
							continue
						else:
							outparam.write("LIMIT CASE NOT COVERED FOR %s"%(f))
					else:
						Z[mfa][f+" meanZ"]=mz
				else:
					mz = (np.nanmean(obs[mfa][f]) - np.nanmean(sm[f]))/simsd
				...
				if (elogv<float(2)*math.log(self.min_sd)):
					"""ADD SMALL NUMBER HERE 10^-10 - or even smaller"""
				...
				# CLIP VERY LARGE values of Z-scores to MIN=-15 and MAX=15
				#if float(Z[mfa][f+" meanZ"])>15:
				#	Z[mfa][f+" meanZ"]=15.0
				#elif float(Z[mfa][f+" meanZ"])<-15:
				#	Z[mfa][f+" meanZ"]=-15.0

				#if float(Z[mfa][f+" varZ"])>15:
				#	Z[mfa][f+" varZ"]=15.0
				#elif float(Z[mfa][f+" varZ"])<-15:
				#	Z[mfa][f+" varZ"]=-15.0

			outres.write(mfa +"\t")
			#print("\t".join([str(Z[mfa][f+" meanZ"]) for f in featurenames + self.repeats.keys()]))
